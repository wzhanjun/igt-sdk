...
namespace Wzhanjun\Igetui\Sdk\Protobuf;

use Wzhanjun\Igetui\Sdk\Protobuf\Type\PBInt;
use Wzhanjun\Igetui\Sdk\Protobuf\Type\PBString;
use Wzhanjun\Igetui\Sdk\Protobuf\Type\PBBool;
use Wzhanjun\Igetui\Sdk\Protobuf\Encoding\Base128varint;
use Wzhanjun\Igetui\Sdk\Protobuf\Reader\PBInputStringReader;
...
abstract class PBMessage
{
    ...
    private function _ParseFromArray($length=99999999)
    {
        ...
        while ($this->reader->get_pointer() - $_begin < $length) {
            ...
            if (is_array($this->values[$messtypes['field']])) {
                $this->values[$messtypes['field']][] = $this->getClass($this->fields[$messtypes['field']]($this->reader));
    ...
    {
        return $this->values[$index][] = $this->getClass($this->fields[$index]);
    }
    ...
    protected function _set_value($index, $value)
    {
        if (gettype($value) == 'object') {
            ...
            $this->values[$index] = $this->getClass($this->fields[$index]);
    ...
    public function getClass($className)
    {
        ...
        if (!class_exists($class)) {
            $class = "\\Wzhanjun\\Igetui\\Sdk\\Igetui\\Req\\" . $className;
        }
